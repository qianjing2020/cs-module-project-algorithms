...
def naive_scavenging(field):
    ...
    list_of_perms = list(product(['se', 'e', 'ne'], repeat=len(field)-1))
    output = ''
    max_gold = 0
...
def dp_scavenging(field):
    ...
    for col in range(length-1, -1, -1):
        for row in range(length):
            ...
            # Update cache with how much gold we collect from choosing
            # the path that will lead to the most gold + the gold from the
            # location we are currently standing in
            # Below, we are NOT using max() function here so that we can
            # also keep track of the path we follow to find this gold
            # OTHERWISE could use...
            # gold_cache[row][col] = field[row][col] + \
            #                         max(east, northeast, southeast)
